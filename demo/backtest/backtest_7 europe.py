############################################################################
### QPMwP CODING EXAMPLES - Backtest 7: ScoreVariance with ML-based signal - Europe
############################################################################

# --------------------------------------------------------------------------
# Cyril Bachelard
# This version:     11.05.2026
# First version:    11.05.2025
# --------------------------------------------------------------------------
...
PATH_TO_DATA = 'C:/Users/User/OneDrive/Documents/QPMwP/Data/Europe/'
SAVE_PATH = 'C:/Users/User/OneDrive/Documents/QPMwP/2026/Code/'
WIDTH_3Y = 365 * 3
...
# --------------------------------------------------------------------------
# Load data
# - return_series.txt: daily returns for each stock, with columns DATE, ID, tot_return_gross
# - signals.csv: features for each stock, with columns date, ID, and various signal
# - msci_europe_ex_ch_index.csv: benchmark index total return series
# --------------------------------------------------------------------------


# Load the daily returns from the txt file, set multi-index and save as parquet for future use
daily_ret = pd.read_csv('C:/Users/User/OneDrive/Documents/QPMwP/Data/Europe/return_series.txt')
daily_ret = daily_ret.set_index(["DATE", "ID"]).sort_index().squeeze()
market_data = (1 + daily_ret.unstack()).cumprod().stack().to_frame(name='price')
market_data.index.names = ['date', 'id']
market_data.index = pd.MultiIndex.from_arrays([
    pd.to_datetime(market_data.index.get_level_values("date")),
    market_data.index.get_level_values("id").astype(str)
    ], names=market_data.index.names)

# Load features from csv file, set multi-index and save as parquet for future use
signals = pd.read_csv('C:/Users/User/OneDrive/Documents/QPMwP/Data/Europe/signals.csv')
signals = signals.set_index(["date", "ID"]).sort_index()
signals.index.names = ["date", "id"]
dates = pd.to_datetime(signals.index.get_level_values("date"), dayfirst=True).strftime("%Y-%m-%d")
signals.index = pd.MultiIndex.from_arrays([
    pd.to_datetime(dates), signals.index.get_level_values("id").astype(str)
    ], names=signals.index.names)

# Load ML signal and add to jkp_data (so that we can use it in the backtest)
ml_signal = pd.read_parquet('C:/Users/User/OneDrive/Documents/QPMwP/Data/Europe/ltr_signal.parquet')
ml_signal = ml_signal.stack().squeeze().astype("float64")
ml_signal.name = 'ml_signal'
ml_signal.index = pd.MultiIndex.from_arrays([
    pd.to_datetime(pd.to_datetime(ml_signal.index.get_level_values(0), dayfirst=True).strftime("%Y-%m-%d")),
    ml_signal.index.get_level_values('ID').astype(str)
], names=['date', 'id'])

# Load benchmark data
bm_data = pd.read_csv('C:/Users/User/OneDrive/Documents/QPMwP/Data/Europe/msci_europe_ex_ch_index.csv')
bm_series = bm_data.set_index('DATE').squeeze()
bm_series.index.name = 'date'
bm_series.index = pd.to_datetime(pd.to_datetime(bm_series.index, dayfirst=True).strftime("%Y-%m-%d"))


# Add the ml_signal to signals
signals = signals.join(ml_signal, how='left')

# Align market data with jkp data
market_data_ffill, signals = align_market_data_with_jkp_data(
...
data.jkp_data = signals
data.bm_series = bm_series
...
rebdates = [
    date for date in rebdates if date >= ml_signal.index.get_level_values('date').min().strftime('%Y-%m-%d')
    and date < rebdates[-1]
]
rebdates
...
# bt_sv.save(
#     path=SAVE_PATH,
#     filename=f'demo_backtest_7_europe_sv_{field_name}.pickle' # <change this to your desired filename>
# )
...
# bt_tqp.save(
#     path=SAVE_PATH,
#     filename=f'demo_backtest_7_europe_tqp_{field_name}.pickle' # <change this to your desired filename>
# )
...
# bt_bqp.save(
#     path=SAVE_PATH,
#     filename=f'demo_backtest_7_europe_bqp_{field_name}.pickle' # <change this to your desired filename>
# )
...
bt_sv = load_pickle(
    filename=f'demo_backtest_7_europe_sv_{field_name}.pickle',
    path=SAVE_PATH,
)
bt_tqp = load_pickle(
    filename=f'demo_backtest_7_europe_tqp_{field_name}.pickle',
    path=SAVE_PATH,
)
bt_bqp = load_pickle(
    filename=f'demo_backtest_7_europe_bqp_{field_name}.pickle',
    path=SAVE_PATH,
)
...
return_series = bs.data.get_return_series(weekdays_only=False, fillna_value=0)
...
sim = sim[sim.index >= '2008-08-01']
sim = sim[sim.index <= '2025-12-31']
